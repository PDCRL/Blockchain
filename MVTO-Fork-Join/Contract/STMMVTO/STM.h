...
struct trans_state
{
	...
	~trans_state()                                                    /*trans_state destructor frees memory allocated to write buffer*/
	{
		...
		for (tb = (read_buffer).begin(); tb != (read_buffer).end(); tb++)
		{
			delete(tb->second);                                       /*invokes tOB destructor*/
		}
		begin_itr = read_buffer.begin();
		end_itr   = read_buffer.end();
		read_buffer.erase(begin_itr, end_itr);
		read_conflist.clear();
	}
};
...
class STM 
{
	...
	virtual list<int> get_conf(long long TID);                        //Print the conflict list corresponding to TID
	/*----------------------------------------------------------*/
