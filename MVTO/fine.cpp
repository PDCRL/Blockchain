...
//		cout<<" Number of Invalid AUs = "+to_string(numAUs-gNodeCount)+"\n";
	...
	static void concMiner( int t_ID, int numAUs, Graph *cGraph)
	{
		...
		//! statrt clock to get 
		//! time taken by this.AU
		auto start = thTimer._timeStart();
		
		while(curInd < numAUs)
		{
			//! trns_id of STM_BTO_trans 
			...
			for(itr = buffer.begin(); itr != buffer.end(); itr++)
			{
				...
				if(temp->in_count == 0)
				{
					...
					if(temp->marked == false )
					{
						...
						//! this node will not be executed 
						//! again:: even mark field 
						//! help but this is optimization
						//! kind of logical delete.
						temp->in_count = -1;
...
int main( )
{
	...
	cout<<pl<<" MVTO Fine Grain Algorithm \n"<<pl;
	int totalRun = 1;
	...
	//! to get total avg miner and 
	//! validator time after number of totalRun runs.
